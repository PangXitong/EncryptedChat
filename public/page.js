...
const vm = new Vue ({
  ...
  async created () {
    this.addNotification('欢迎！现在生成一个新的关键点对。')
    ...
    this.addNotification(`生成密钥对 - ${this.getKeySnippet(this.originPublicKey)}`)
  ...
  methods: {
    ...
    setupSocketListeners () {
      ...
      this.socket.on('NEW_CONNECTION', () => {
        this.addNotification('另一个用户加入了房间')
        this.sendPublicKey()
      })
      ...
      this.socket.on('ROOM_JOINED', (newRoom) => {
        ...
        this.addNotification(`加入房间 - ${this.currentRoom}`)
        this.sendPublicKey()
      })

      // Save public key when received
      this.socket.on('PUBLIC_KEY', (key) => {
        this.addNotification(`已接收公钥 - ${this.getKeySnippet(key)}`)
    ...
    joinRoom () {
      if (this.pendingRoom !== this.currentRoom && this.originPublicKey) {
        this.addNotification(`连接到房间 - ${this.pendingRoom}`)
